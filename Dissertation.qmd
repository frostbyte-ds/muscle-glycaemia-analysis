...
Following the creation of this large dataset was the handling of structural missingness and harmonisation of lifestyle confounders, the first of which being physical activity, which is well known to be strongly associated with skeletal muscle mass and glycaemic control @phys_gluc_2018 @phys_musc_2023. The physical activity variable used for modelling combined total minutes of "moderate" and "vigorous" work-related and recreational activity, along with total minutes of travelling by walking or cycling, in a typical day (definitions of these activity levels are provided in the NHANES survey codebooks).

The physical activity questionnaires in NHANES contain questions that first ask whether the participant takes part in any moderate/vigorous activity during work or recreationally, stored in the variables `PAQ665/PAQ650` and `PAQ620/PAQ605`, respectively. If the participant gives the answer "Yes", they will also be asked how many minutes of the appropriate level of activity they carry out in a typical day - `PAD675/PAD660` for moderate/vigorous recreational activity and equally `PAD630/PAD615` for work-related activity.

Similarly, participants were asked whether they walk or cycle to travel for more than ten minutes in a typical week (`PAQ665`). For the 2011-12 survey cycle, the one week time-frame was not specified in the question, but it is implied for this analysis for simplicity. Only if they answered "Yes" were they asked the follow-up question of how many minutes of this travel-related activity they conduct in a typical day (`PAD645`).

This structure introduces structural missingness. Any time a participant answered "No" to the initial gatekeeper question, they were not asked the follow up question, resulting in the skipped response being coded as `NA` in the data. Of course, this data is not "missing". An initial answer of "No" should correspond to 0 minutes of the appropriate level of physical activity in a typical day. This issue was appropriately handled as seen in Section \ref{procCode}. Very few individuals also responded with "Don't know" when asked the follow-up question regarding minutes of activity. This was coded as 9999 in the dataset and was handled by setting 9999 values to `NA` so that they could be imputed. Handling the outlined structural missingness resulted in the proportion of missing data for `PAD675`, `PAD660`, `PAD630`, `PAD615`, and `PAD645` reducing from 56%, 69%, 60%, 77%, and 71%, respectively, to 0.09%, 0.04%, 0.3%, 0.2%, and 0.2%, emphasising the importance of this data processing. The final physical activity variable was formed by summing `PAD675`, `PAD660`, `PAD630`, `PAD615`, and `PAD645`.

Some values of this final physical activity variable were implausibly high. It is not possible to discern why this was the case, but a likely estimate would be due to questions being misunderstood by participants. It was made clear in the questions of the physical activity questionnaire to exclude activity included in previous questions. For example, when asked about work-related activity, participants were told that "the next questions exclude the physical activity at work that you have already mentioned." This instruction may have not been followed in cases of extreme combined activity values.
...
This multicollinearity had different consequences for each outcome. For the Onset outcome, the null effect of ALMI was revealed as soon as waist circumference was added regardless of whether body fat percentage was included, suggesting that central adiposity is the primary confounder of the initial paradoxical association.
...
Specifically, the table contains the mean of the mean performance metrics over each of the 10 repeats, as a way to concisely summarise the overall performance of the model. The most important statistic, the mean weighted AUC of 0.844, suggests that the model has high discriminatory power.

The mean optimal threshold of 0.061, although not a measure of model performance, is useful to be aware of. This means that on average, an observation was classified as diabetic if they were predicted a probability of either 0.061 or higher of being so. This threshold provided the optimal balance between sensitivity and specificity, resulting in the respective means of 79.1% and 74.8%. The mean overall weighted accuracy was 75.1%.
...
#| out-width: "80%"
#| fig-align: "center"
#| fig-cap: "Receiver Operating Characteristic (ROC) curve from the final iteration of the cross-validation for Progression Model 4. The orange diamond highlights the optimal threshold selected by maximising Youden's J statistic. The Area Under the Curve (AUC) of 0.85 indicates high discriminatory power."
