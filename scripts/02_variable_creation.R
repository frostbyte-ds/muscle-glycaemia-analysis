...
vars1112 <- c(
  ...
  "PAQ620", "PAD630", "PAQ665", "PAD675", "PAQ605", "PAD615", "PAQ650", "PAD660",  # Physical activity
...
# Also doing so for variables relating to work-related physical activity
...
imputation_vars_1118 <- imputation_vars_full.4 %>%
  mutate(Phys = PAD660 + PAD675 + PAD630 + PAD615,
         Height_m = BMXHT / 100) %>%
  select(-c(BMXHT, PAD660, PAD675, PAD630, PAD615)) %>%
  rename(
    ID = SEQN,
    PSU = SDMVPSU,
    Strata = SDMVSTRA,
